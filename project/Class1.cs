...
namespace project
{
    ...
    public class Class1
    {
        ...
        public void insert_employee(string unm, string pwd, string nm, string address, string mbl, string eml, string cnt)
        {
            getcon();
            cmd = new SqlCommand("insert into Employers_tbl(UserName, Password, FullName, Address, Mobile, Email, Country) values" +
                    " ('" + unm + "','" + pwd + "','" + nm + "','" + address + "'," +
                    "'" + mbl + "','" + eml + "','" + cnt + "') ", con);
            cmd.ExecuteNonQuery();
            con.Close();
